...
- 直近7日の絞り込みは種類判定の**あと**に1回だけ掛かります（`popup.js` の `pub`）。基準は `time`＝視聴可能になった時刻で、`upcoming` は対象外です。RSS 取得の段階では日付で足切りしません。`published` は枠を立てた時刻で配信が始まっても動かないため、そこで切ると古い枠の配信中が消えるからです（v1.0.4 で修正）。この前段の足切りを戻さないでください。
